...
F1 = A1*z;
sagbi_homotopy(F1, sagbi1, weight = w1);
...
for m=4:j
    for i=2:floor(Int, m/2)
        ...
        result, elapsed_time, allocations, _, _ = @timed sagbi_homotopy(F, sagbi, weight = w)
        #open("SlicingGrassmannian/Grassmannians_timing_log_sagbi.txt", "a") do io
        #    println(io, "k = ", k, ", ", "m = ", m)
