...
F1 = A1*z - q;
sagbi_homotopy(F1, vcat(l.*vcat(1,x1),sagbi1));
...
for m=4:j
    for i=2:floor(Int, m/2)
        ...
        @var l
        result, elapsed_time, allocations, _, _ = @timed sagbi_homotopy(F, vcat(l.*vcat(1,x),sagbi))
        open("CCEquations/CC_timing_log_sagbidetection.txt", "a") do io
            println(io, "k = ", k, ", ", "m = ", m)
